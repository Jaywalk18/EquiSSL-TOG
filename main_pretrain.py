...
Usage:
    python main_pretrain.py --config configs/pretrain_v9_repaired.yaml

    # Multi-GPU:
    torchrun --nproc_per_node=3 main_pretrain.py --config configs/pretrain_v9_repaired.yaml
"""
...
from equissl.utils.paths import dataset_dir
...
def parse_args():
    ...
    parser.add_argument("--config", type=str, default="configs/pretrain_v9_repaired.yaml")
...
def build_dataset(cfg, split="train", max_samples=None):
    ...
    dataset = Structured3DSSL(
        root_dir=dataset_dir(data_cfg["root_dir"], "STRUCTURED3D_PATH"),
